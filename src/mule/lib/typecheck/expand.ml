...
let rec walk_q ctx qv ~g ~parents ~is_root ~inst_c =
  let q = Context.read_var ctx Context.quant qv in
  ...
  Lazy.force qv'
and walk_ty ctx ~tv ~parents ~root ~g ~inst_c =
  let ty = Context.read_var ctx Context.typ tv in
  let walk_q' qv = walk_q ctx qv ~parents ~is_root:(NotRoot root) ~g ~inst_c in
...
let expand ctx ~g ~at ~inst_c =
  ...
  walk_q ctx qv ~g:at ~parents ~is_root:(Root g) ~inst_c
